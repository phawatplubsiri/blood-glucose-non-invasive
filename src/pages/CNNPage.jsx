...
export default function CNNPage() {
  ...
  const selectedModel = "training_results_1DCNN";
  // state สำหรับเก็บ 'run' ที่ผู้ใช้เลือกจาก Dropdown (ค่าเริ่มต้น 'run001')
  const [selectedRun, setSelectedRun] = useState("run001");
  // state สำหรับเก็บ 'chart' ที่ผู้ใช้เลือก เพื่อกรองการแสดงผล (ค่าเริ่มต้น 'all')
      ...
      <div className="my-1 flex flex-col items-center gap-4 lg:flex-row lg:items-start lg:justify-center">
        ...
        <div className={`transition-all duration-300 bg-white p-3 shadow-md border rounded-xl space-y-4 ${
          ...
          {/* Conditional Rendering: 
            ...
            ถ้า `selectedChart` เปลี่ยนไป คอมโพเนนต์ที่จะแสดงผลก็จะเปลี่ยนตาม
          */}
        ...
        <ControlPanel isOpen={isControlOpen} setIsOpen={setIsControlOpen}>
          ...
          <div className="pt-2">
            <ColabDriveLinks
              colabLink = {import.meta.env.VITE_COLAB_CNN}
              driveLink = {import.meta.env.VITE_DRIVE_CNN}
            />
          </div>
        </ControlPanel>
      </div>
